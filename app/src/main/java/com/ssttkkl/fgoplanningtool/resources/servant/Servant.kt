...
data class Servant(val id: Int,
    ...
    val ascensionQP: List<Long>
        get() =
            if (id == 1) listOf(0, 0, 0, 0) else QPValues.ascensionQP[star]

    val skillQP: List<Long>
        get() =
            QPValues.skillQP[star]

    val stageMapToMaxLevel: List<Int>
        get() =
            if (id == 1)
                LevelValues.stageMapToMaxLevel[star].slice(0..4)
            else
                LevelValues.stageMapToMaxLevel[star]

    fun calcExpCardQP(curLevel: Int): Long {
        val (a, b) = QPValues.expCardQP[star]
        return a + b * curLevel
    }
}