...
import kotlinx.android.synthetic.main.fragment_servantdetail.*

class ServantDetailDialogFragment : DialogFragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        return inflater.inflate(R.layout.fragment_servantdetail, container, false)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewPager.adapter = object : FragmentPagerAdapter(childFragmentManager) {
            ...
            override fun getItem(position: Int): Fragment = ServantDetailLevelListFragment()
            ...
            override fun instantiateItem(container: ViewGroup, pos: Int): Any {
                return (super.instantiateItem(container, pos) as ServantDetailLevelListFragment).apply {
                    data = if (pos == 0)
    ...
    private fun generateEntities(getStart: (Int) -> String,
                                 ...
                                 getQP: (Servant?) -> List<Long>): List<ServantDetailLevelListEntity> {
        val qp = getQP(servant)
        val items = getItems(servant)
        return items.mapIndexed { idx, it -> it + Item("qp", qp[idx]) }
                .mapIndexed { idx, it ->
                    ServantDetailLevelListEntity(start = getStart.invoke(idx),
                            to = getTo.invoke(idx),
    ...
    companion object {
        fun newInstance(servantID: Int) = ServantDetailDialogFragment().apply {
