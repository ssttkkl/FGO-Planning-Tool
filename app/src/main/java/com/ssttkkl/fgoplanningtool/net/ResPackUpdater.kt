...
class ResPackUpdater(val context: Context): OnDownloadListener {
    interface Callback {
    ...
    override fun onDownloadComplete() {
        callback?.onDownloadComplete(File(downloadRequest.dirPath, downloadRequest.fileName))
    }
    ...
    private val work = launch(Dispatchers.net, CoroutineStart.LAZY) {
        val latestInfoFile = File(this@ResPackUpdater.context.cacheDir, "${UUID.randomUUID()}.json").apply { deleteOnExit() }
        try {
        ...
        val resPackFile = File(this@ResPackUpdater.context.cacheDir, "${UUID.randomUUID()}.zip").apply { deleteOnExit() }
        downloadRequest = PRDownloader.download(UpdateResPackDialogFragment.urlPattern.format(latestInfo.filename), resPackFile.parent, resPackFile.name).build()
                .setOnProgressListener {
                    callback?.onDownloadProgress(((it.currentBytes.toDouble() / it.totalBytes.toDouble()) * 100).toInt())
                }
        downloadRequest.start(this@ResPackUpdater)
    }
